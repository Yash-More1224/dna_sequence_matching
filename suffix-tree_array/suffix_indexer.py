...
class SuffixIndexer:
    ...
    def __init__(self, text: str):
        """
        Initialize the SuffixIndexer with a DNA sequence.
        
        Args:
            text (str): Input DNA sequence (should contain only A, C, G, T, N)
        
        The constructor automatically builds the suffix array and LCP array.
        """
        self.text = text
        self.sa: List[int] = []
        self.lcp: List[int] = []
    ...
    def build_index(self, text: str) -> Tuple[float, int]:
        ...
        print(f"[SuffixIndexer] Index built successfully")
...
def demo_usage():
    """
    Demonstrate basic usage of the SuffixIndexer class.
    """
    print("=" * 70)
    print("SuffixIndexer Demo - DNA Pattern Matching")
    print("=" * 70)
    
    # Example 1: Classic "banana" example
    print("\n1. Classic 'banana' example:")
    print("-" * 70)
    indexer = SuffixIndexer("banana")
    print(f"Suffix Array: {indexer.sa}")
    print(f"LCP Array: {indexer.lcp}")
    
    matches = indexer.search_exact("ana")
    print(f"\nSearching for 'ana': found at positions {matches}")
    
    matches = indexer.search_exact("na")
    print(f"Searching for 'na': found at positions {matches}")
    
    # Example 2: DNA sequence with repeats
    print("\n\n2. DNA sequence with repeats:")
    print("-" * 70)
    dna = "AGATTTAGATTAGCTAGATTA"
    indexer = SuffixIndexer(dna)
    
    matches = indexer.search_exact("AGATTA")
    print(f"Searching for 'AGATTA' in {dna}")
    print(f"Found at positions: {matches}")
    
    print("\nFinding repeats (min_length=4):")
    repeats = indexer.find_longest_repeats(min_length=4)
    for i, repeat in enumerate(repeats[:5], 1):
        print(f"  {i}. '{repeat['substring']}' (len={repeat['length']}, "
              f"count={repeat['count']}) at positions {repeat['positions']}")
    
    # Example 3: Larger DNA sequence
    print("\n\n3. Larger DNA sequence (100 bases):")
    print("-" * 70)
    dna_large = "ATCGATCGATCG" * 8 + "GGGGCCCC" + "ATCGATCG" * 2
    indexer = SuffixIndexer(dna_large)
    
    pattern = "ATCGATCG"
    matches = indexer.search_exact(pattern)
    print(f"Searching for '{pattern}' in {len(dna_large)}-base sequence")
    print(f"Found {len(matches)} occurrences at positions: {matches[:10]}..." 
          if len(matches) > 10 else f"Found {len(matches)} occurrences at positions: {matches}")
